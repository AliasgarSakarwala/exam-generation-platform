...
import { createClassroom, updateClassroom, CreateClassroomParams } from "../../../services/classroom";
...
export default function Modal({
    ...
    const [courseData, setCourseData] = useState(() => ({
        ...
        class_colour: initialData?.class_colour || getRandomColor(),
    ...
    const randomizeColor = () => {
        setCourseData(prev => ({ ...prev, class_colour: getRandomColor() }));
    };

    const validateCourseCode = (code: string) => {
        const trimmedCode = code.trim();
        return /^[A-Za-z]{3,4}-\d{3}$/.test(trimmedCode);
    };

    const validateSection = (section: string) => {
        if (!section) return false;
        return /^[0-9]{1,3}$/.test(section);
    };
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (!courseData.section) {
            toast.error("Please enter a section number");
            return;
        }
        if (!validateSection(courseData.section.toString())) {
            toast.error("Section number should be 1-3 digits");
            return;
        }
        ...
        try {
            const classroomData: CreateClassroomParams = {
                ...
                student_count: mode === 'create' ? 0 : courseData.student_count,
            };
            ...
            if (mode === 'create') {
                ...
                response = await createClassroom(classroomData);
                console.log("Response from createClassroom:", response);
                console.log("Response data:", response.data);
                classroomId = response.data.classroom_id || response.data.id;

                if (!classroomId) {
                    throw new Error("Classroom was created but no ID was returned");
                }
                ...
                // Reset form and close modal after a small delay
                setTimeout(() => {
                    setCourseData({
                        name: "",
                        code: "",
                ...
                response = await updateClassroom(initialData!.courseId, classroomData);
            ...
            if (onSubmit) {
                onSubmit({
                    ...courseData,
                    courseId: mode === 'create' ? classroomId : initialData!.courseId,
                });
            }
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : `Failed to ${mode} course`;
            console.error(`Error ${mode}ing course:`, err);
            toast.error(`Failed to ${mode} course: ${errorMessage}`);
        } finally {
    ...
    if (mode === 'edit' && !initialData) {
        console.error('EditModal: initialData is required for edit mode');
        return null;
    }
                ...
                >
                    ...
                    <div className={`px-6 py-4 rounded-t-xl ${darkMode ? "bg-zinc-700" : "bg-gray-100"}`}>
                        <h2 className="text-lg font-semibold">
                            <span style={{ color: '#3774E5' }}>{mode === 'create' ? 'Add New Course' : 'Edit Course'}</span>
                        </h2>
                    </div>
                    ...
                    <form onSubmit={handleFormSubmit} className="p-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    <label className={`block mb-2 text-sm font-medium ${darkMode ? "text-gray-300" : "text-gray-700"}`}>
                                        Section No. <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="text"
                                        name="section"
